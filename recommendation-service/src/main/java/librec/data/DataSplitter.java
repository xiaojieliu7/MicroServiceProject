...
public class DataSplitter {
     ...
     *
     * @param rateMatrix data matrix
     * @param kfold      number of folds to split
     */
     ...
     * @param timestamps the timestamps of all rating data
     */
    public SparseMatrix[] getRatioByRatingDate(double ratio, SparseMatrix timestamps) {
        ...
        List<GenericRatingContext> rcs = new ArrayList<>(timestamps.size());
        int u, i, j;
        long timestamp;
        for (MatrixEntry me : rateMatrix) {
            ...
            rcs.add(new GenericRatingContext(u, i, timestamp));
        }
        ...
        for (i = 0; i < rcs.size(); i++) {
            GenericRatingContext rc = rcs.get(i);
            u = rc.getUser();
            j = rc.getItem();
     ...
     *
     * @param ratio      the ratio of training data
     * @param timeMatrix the timestamps of all rating data
     */
    public SparseMatrix[] getRatioByUserDate(double ratio, SparseMatrix timeMatrix) {
        ...
        for (int user = 0, um = rateMatrix.numRows; user < um; user++) {
            ...
            List<GenericRatingContext> rcs = new ArrayList<>(size);
            for (int item : unsortedItems) {
                rcs.add(new GenericRatingContext(user, item, (long) timeMatrix.get(user, item)));
            }
            Collections.sort(rcs);
    ...
    public SparseMatrix[] getRatioByItemDate(double ratio, SparseMatrix timestamps) {
        ...
        for (int item = 0, im = rateMatrix.numColumns; item < im; item++) {
            ...
            for (int user : unsortedUsers) {
                rcs.add(new GenericRatingContext(user, item, (long) timestamps.get(user, item)));
            }
            Collections.sort(rcs);
     ...
     * @param validRatio validation ratio
     */
    ...
    public SparseMatrix[] getLOOByItem(boolean isByDate, SparseMatrix timestamps) throws Exception {
        ...
        for (int i = 0, im = rateMatrix.numColumns(); i < im; i++) {
            ...
            if (!isByDate) {
                ...
                List<GenericRatingContext> rcs = new ArrayList<>();
                for (int v : users) {
                    rcs.add(new GenericRatingContext(v, i, (long) timestamps.get(v, i)));
                }
                Collections.sort(rcs);
    ...
    public SparseMatrix[] getGivenNByUserDate(int numGiven, SparseMatrix timestamps) throws Exception {
        ...
        for (int u = 0, um = rateMatrix.numRows(); u < um; u++) {
            ...
            List<GenericRatingContext> rcs = new ArrayList<>(capacity);
            for (int j : items) {
                rcs.add(new GenericRatingContext(u, j, (long) timestamps.get(u, j)));
            }
            Collections.sort(rcs);
     ...
     *
     * @param numUsers number of users, -1 to use all users;
     * @param numItems number of items, -1 to user all items;
     */
