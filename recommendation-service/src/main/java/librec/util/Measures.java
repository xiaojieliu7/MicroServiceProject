...
public class Measures {
     ...
     */
    public static <T> double AP(List<T> rankedList, Set<T> groundTruth) {
        int hits = 0;
        double sumPrecs = 0;
        ...
        for (int n = 0, m = rankedList.size(); n < m; n++) {
            ...
            if (groundTruth.contains(item)) {
                hits++;
                sumPrecs += hits / (n + 1.0); // prec@n
            }
        }

        if (hits > 0)
            return sumPrecs / groundTruth.size();
        else
            return 0.0;
    }
    ...
    public static <T> int HitsAt(List<T> rankedList, Set<T> groundTruth, int n) {
        ...
        for (int i = 0, k = rankedList.size() > n ? n : rankedList.size(); i < k; i++) {
     ...
     */
    public static <T> double nDCG(List<T> rankedList, Set<T> groundTruth) {
        double dcg = 0;
     ...
     */
    public static <T> double RR(List<T> rankedList, Set<T> groundTruth) {
     ...
     * @param testSetByUser
     * @param groundTruth
     * @param numDroppedItems
     * @param <T>
     * @return
     */
    public static <T> double AUC(Set<T> testSetByUser, Set<T> groundTruth, int numDroppedItems) {
        double auc = 0.0d;
        int numRelevantItems = 0, numMissItems = 0;
        for (T testItemIdx : testSetByUser) {
            if (groundTruth.contains(testItemIdx)) {
                numRelevantItems++;
            } else {
                numMissItems++;
            }
        ...
        int numEvaluatingItems = groundTruth.size() + numDroppedItems;
